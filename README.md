# Testing `mcp-test-server` (and mcp-test-server-standard-stateless)
...
**Order matters** — `initialize` must run first (it creates the session, if a session id is required), the
...
**Scripts tab → Post-response**, add this so the session ID is captured automatically (in the case of a stateful server):
...
Or copy the session ID from the response header Mcp-Session-Id to the header of the new request (same header name as the reponse).

---

### Request 2 — `MCP Tools List`
...
Expected response: a `tools` array specific for the test mcp server. In the case of mcp-tet-server, listing `get_time`, `search`, `generate_report`, and
`run_heavy_task`, each with its `inputSchema`.

These tools need to be added in the merchan Create/Edit service form in order for the consumer to be charged - if not already there. 
---

### Request 4 — `MCP Tool Call`

Same headers as Request 3. Swap the body depending on which tool you're testing (for example, in the case of the test-mcp-server):
